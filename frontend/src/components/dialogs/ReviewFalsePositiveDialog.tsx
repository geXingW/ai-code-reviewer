...
export function ReviewFalsePositiveDialog({
  ...
  const noteValid = action === 'confirm' ? true : trimmedNote.length >= MIN_NOTE_LEN;
  ...
  return (
    <Modal
      ...
      title={title}
      okText={action === 'confirm' ? '确认误报' : '驳回'}
      cancelText="取消"
      okButtonProps={{ disabled: !canSubmit, loading: submitting, danger: action === 'reject' }}
      ...
      <div className="space-y-3 pt-2">
            ...
            <div className="mt-2 space-y-1 border-t border-zinc-200 pt-2">
              <div className="text-[11px] text-zinc-500">
                ...
                <span className="text-zinc-800">
                  {relativeTime(finding.fp_marked_at ?? undefined) || '未知'}
                </span>
              </div>
              <div>
                <div className="mb-1 text-[11px] text-zinc-500">提交原因</div>
                <div className="whitespace-pre-wrap break-words rounded border border-zinc-200 bg-white px-2 py-1.5 font-mono text-[12px] text-zinc-800">
        ...
        {action === 'confirm' ? (
          <Alert
            type="info"
            showIcon
            message="确认后本 finding 的代码片段将作为负例注入到该规则未来的评审 prompt 中。"
          />
        ) : (
          <Alert type="warning" showIcon message="驳回后 finding 状态回到普通问题，标记人可继续与规则维护者沟通。" />
        )}

        <div className="space-y-1.5">
          <label className="text-[12px] font-medium text-zinc-600" htmlFor="fp-review-reviewed-by">
            审核人
          </label>
          <Input
        ...
        <div className="space-y-1.5">
          <label className="text-[12px] font-medium text-zinc-600" htmlFor="fp-review-note">
            审核备注
            {action === 'reject' ? (
              <span className="ml-1 text-rose-600">*</span>
            ) : (
              <span className="ml-1 font-normal text-zinc-400">（建议填写）</span>
            )}
          </label>
          <Input.TextArea
            id="fp-review-note"
