...
def build_push_review_note(
    *,
    ...
    """渲染一次 push 合并审查的 GitLab 汇总评论（挂在 head commit 下）。

    风格对齐 :func:`build_commit_review_note`（emoji 标题、severity/category
    分布、复用 ``_render_findings_section`` 渲染 findings 列表），但范围是
    **一次 push 的全部 commit 变更**（compare before..after 合并 diff 后的
    单次审查），因此顶部信息行改为分支 + head commit + commit 数，并列出
    本次 push 的 commit 标题列表供上下文参考。

    Args:
        review_id: 评审记录 UUID。
        head_sha: push 后 head commit SHA（写回目标）。
        branch: push 目标分支名。
        commit_count: 本次 push 携带的 commit 数。
        commit_titles: 本次 push 的 commit 标题列表（旧 -> 新）。
        findings: 引擎产出的 findings。
        has_blocker: 是否命中阻断策略。
        blocker_count: 阻断级 finding 数。
        policy_applied: 命中的阻断策略描述（如 ``"* -> NONE"``）。
    ...
    if engine_error is not None:
        lines = [
            "# ⚠️ AI Review FAILED",
            "",
            "**本次 push 的 AI 审查未能完成，请人工审查这次提交。**",
            "",
            f"- Review ID: `{review_id}`",
            f"- Branch: `{branch}`",
            f"- Head commit: `{sha_short}`",
    ...
    non_empty_titles = [title for title in commit_titles if title]
    if non_empty_titles:
        output.extend(["", "**本次 push 的 commits：**", ""])
        output.extend(f"- {title}" for title in non_empty_titles)
