...
These models intentionally live separately from ``schemas`` (which
...
"""Severity levels mirrored from ``schemas.finding.Severity``."""
...
class FindingSource(StrEnum):
    ...
    ``LLM_INFERRED`` 表示 LLM 自己发挥出来的 finding：没有明确的团队规则
    背书。这是默认值，也是 Filter 阶段应该最激进证伪的一档。
    """
...
class Finding(BaseModel):
    ...
    DB rows. Field names align with :class:`schemas.finding.FindingCreate`
