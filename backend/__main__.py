"""CLI entrypoint for ``python -m app``.
...
Usage::
    ...
    python -m app                    # start on 0.0.0.0:8000, load ./.env
...
def _load_env_file(env_file: Path) -> None:
    ...
    shell-expanded; they are set verbatim. Existing environment variables
    take precedence (we never overwrite).
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Load .env file before importing app so settings pick up the values
    if args.env_file:
